...
class Config:
    ...
    def __init__(self, agent_class=None, env_class=None, env_args=None):
        ...
        self.edge_index = env_args['edge_index']
        self.edge_attr = env_args['edge_attr']
    ...
    def get_if_off_policy(self) -> bool:
        ...
        on_policy_names = ('SARSA', 'VPG', 'A2C', 'A3C', 'TRPO', 'MPO', 'PPO', 'GraphPPO','GraphPPO_','TAGConvPPO_','GATPPO_','GATPPO_RX')
...
class ReplayBufferGraph:  # for off-policy
    """
    Replay Buffer for storing and sampling experiences for off-policy reinforcement learning algorithms.
    ...
    def __init__(self,
        ...
        """
        self.states = torch.empty((max_size, num_seqs, state_dim), dtype=torch.float32, device=self.device)
        self.node_features = torch.empty((max_size, num_seqs, node_num, num_features),
    ...
    def update(self, items: Tuple[Tensor, ...]):
        ...
        """
        self.add_item = items
        states, node_features, actions, rewards, undones = items

        assert states.shape[1:] == (self.args.num_envs, self.args.state_dim)
        if node_features.ndim == 3:
            node_features = node_features.unsqueeze(1)  # (horizon, 1, node_num, features)
        assert node_features.shape[1:] == (self.args.num_envs, self.node_num, self.num_features), \
            f"Expected node_features shape (*, {self.args.num_envs}, {self.node_num}, {self.num_features}), got {node_features.shape}"
        assert actions.shape[1:] == (self.args.num_envs, self.args.action_dim)
        assert rewards.shape[1:] == (self.args.num_envs,)
        assert undones.shape[1:] == (self.args.num_envs,)
        self.add_size = rewards.shape[0]
        ...
        if p > self.max_size:
            ...
            self.states[p0:p1], self.states[0:p] = states[:p2], states[-p:]
            self.node_features[p0:p1], self.node_features[0:p] = node_features[:p2], node_features[-p:]
            self.actions[p0:p1], self.actions[0:p] = actions[:p2], actions[-p:]
            self.rewards[p0:p1], self.rewards[0:p] = rewards[:p2], rewards[-p:]
            self.undones[p0:p1], self.undones[0:p] = undones[:p2], undones[-p:]
        else:
            self.states[self.p:p] = states
            self.node_features[self.p:p] = node_features
            self.actions[self.p:p] = actions
            self.rewards[self.p:p] = rewards
            self.undones[self.p:p] = undones
    ...
    def sample(self, batch_size: int) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor]:
        ...
        return (self.states[ids0, ids1],
                self.node_features[ids0, ids1],
                self.actions[ids0, ids1],
                self.rewards[ids0, ids1],
                self.undones[ids0, ids1],
                self.states[ids0 + 1, ids1],
                self.node_features[ids0 + 1, ids1],)  # next_state
    ...
    def sample_for_per(self, batch_size: int) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor]:
        ...
        return (
            ...
            self.undones[ids0, ids1],
            self.states[ids0 + 1, ids1],  # next_state
            self.node_features[ids0 + 1, ids1],
            is_weights,  # important sampling weights
            is_indices,  # important sampling indices
        )
...
def get_episode_return_graph(env, act, edge_index, device):
    """
    Calculates the return of an episode.
    ...
    for i in range(96):
        node_features = env._state_to_node_features(state).to(device)
        a_tensor = act(node_features, edge_index)
...
def get_episode_return_graph_(env, act, edge_index, device):
    """
    Calculates the return of an episode.
    ...
    for i in range(96):
        node_features,price_n_time = env._state_to_node_features_(state)
        node_features = node_features.to(device)
        price_n_time = price_n_time.to(device)
        a_tensor = act(node_features, price_n_time, edge_index)
