...
from rl_adn.environments.battery import Battery,battery_parameters_34, battery_parameters_69
...
class PowerNetEnv(gym.Env):
    ...
    def __init__(self, env_config: dict = env_config) -> None:
        ...
        print('network_info', self.network_info)
        ...
        if self.node_num == 34:
            battery_parameters = battery_parameters_34
        elif self.node_num == 69:
            battery_parameters = battery_parameters_69
        else:
    ...
    def _get_edge_index(self): #TODO: check the edge index
        """
        Get the edge index for the network.

        """
        network_branch_info = pd.read_csv(self.network_info['branch_info_file'])
        edge_index = network_branch_info[['FROM', 'TO']].values.T
        edge_index_in = network_branch_info[['TO', 'FROM']].values.T
        edge_index -= 1
        edge_index_in -=1
        edge_index_ = np.concatenate((edge_index, edge_index_in), axis=1)
        edge_index_ = torch.tensor(edge_index_, dtype=torch.long)

        return edge_index_

    def _get_edge_attr(self): #TODO: check the edge index
        """
        Get the edge index for the network.

        """
        network_branch_info = pd.read_csv(self.network_info['branch_info_file'])
        edge_attr = network_branch_info[['R', 'X']].values
        edge_attr_ = np.concatenate((edge_attr, edge_attr), axis=0)

        min_vals = edge_attr_.min(axis=0)
        max_vals = edge_attr_.max(axis=0)
        edge_attr_norm = (edge_attr_ - min_vals) / (max_vals - min_vals + 1e-8)

        edge_attr_norm = torch.tensor(edge_attr_norm, dtype=torch.float32)

        return edge_attr_norm
    ...
    def _state_to_node_features(self, state): #TODO: check the node features

        net_load, soc_all, price, current_time, vm_pu_battery_nodes = self._split_state(state)

        net_load = np.array(net_load)
        soc_nodes = np.zeros(self.node_num)
        price = np.full(self.node_num, price[0])

        current_time = np.full(self.node_num, current_time)
        vol_battery_nodes = np.zeros(self.node_num)
        ess_nodes = np.zeros(self.node_num)

        soc_nodes[self.battery_list] = soc_all
        vol_battery_nodes[self.battery_list] = vm_pu_battery_nodes
        ess_nodes[self.battery_list] = 1

        features = np.stack((net_load, soc_nodes, price, current_time, vol_battery_nodes, ess_nodes), axis=1)
    ...
    def _get_obs(self):
        ...
        if self.state_pattern == 'default':
            ...
            if self.algorithm == "Laurent":
                ...
                active_power = cp.copy(one_slot_data[0:self.node_num]) # scale the active power to fit the 123 node network
                ...
                active_power[0] = 0
                renewable_active_power = one_slot_data[self.node_num:68]
    ...
    def _apply_battery_actions(self, action):
        ...
        if self.state_pattern == 'default':
            if self.algorithm == "Laurent":
                ...
                vm_pu_before_control = cp.deepcopy(abs(v_totall))
                self.vm_pu_before_control = vm_pu_before_control
                vm_pu_before_control_bat = np.squeeze(vm_pu_before_control)[self.battery_list]
                current_each_node = np.matmul(self.dense_Ybus, v_totall)
                power_imported_from_ex_grid_before = current_each_node[0].real
                self.power_imported_from_ex_grid_before = power_imported_from_ex_grid_before

                energy_change_list = np.zeros(self.node_num)
                for i, node_index in enumerate(self.battery_list):
                    ...
                    energy_change_list[node_index] = getattr(self, f"battery_{node_index}").energy_change
                self.solution = self.net.run_pf(active_power=self.active_power)
                self.energy_change_list = energy_change_list
                ...
                self.power_imported_from_ex_grid_after = power_imported_from_ex_grid_after
                saved_energy = power_imported_from_ex_grid_before - power_imported_from_ex_grid_after
                self.saved_cost = saved_energy * self.price
                self.battery_profit = -0.001 * np.sum(energy_change_list * self.price)  # TODO: check the battery profit
            else:
        ...
        return power_imported_from_ex_grid_after, saved_energy, vm_pu_before_control_bat, vm_pu_after_control_bat
    ...
    def step(self, action: np.ndarray) -> tuple:
        ...
        power_imported_from_ex_grid_after, saved_energy, vm_pu_before_control_bat, vm_pu_after_control_bat = self._apply_battery_actions(action)
        self.operation_cost = self.power_imported_from_ex_grid_after * self.price
    ...
    def _calculate_reward(self, current_normalized_obs: np.ndarray, vm_pu_after_control_bat: np.ndarray,
        ...
        """
        if self.state_pattern == 'default':
            reward_for_power = 2 * current_normalized_obs[self.node_num + len(self.battery_list)] * float(saved_power) #*2 for34; *4 for69
            reward_for_penalty = 0.0

            for vm_pu_bat in vm_pu_after_control_bat:
                reward_for_penalty += min(0, 100 * (0.05 - abs(1.0 - vm_pu_bat)))  # penalty for voltage violation
...
if __name__ == '__main__':
    ...
    state = power_net_env.reset()
    print("Training dates:", power_net_env.data_manager.train_dates)
    print("Testing dates:", power_net_env.data_manager.test_dates)
    node_features = power_net_env._state_to_node_features_(state)
    edge_index = power_net_env._get_edge_index()
    edge_attr = power_net_env._get_edge_attr()
    day_data = power_net_env.data_manager.select_day_data(power_net_env.year, power_net_env.month, power_net_env.day)
    for j in range(1):
        episode_reward = 0
        for i in range(24):
            # 1 is charge -1 is discharge
            tem_action = np.ones(len(power_net_env.battery_list))
            print("action", tem_action)
            print('year, month, day, current time',
                  (power_net_env.year, power_net_env.month, power_net_env.day, power_net_env.current_time))
            next_state, reward, finish, info = power_net_env.step(tem_action)
            print('next_obs', next_state)
