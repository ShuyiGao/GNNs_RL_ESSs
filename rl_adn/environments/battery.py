...
class Battery():
    ...
    def step(self, action_battery):
        ...
        Description:
            ...
            The energy change is also used to calculate the cost associated with battery operation.
        """
        self.energy = action_battery * self.max_charge
        updated_soc = max(self.min_soc,min(self.max_soc, ((self.current_soc * self.capacity + self.energy * 15 / 60) / self.capacity)))

        self.energy_change = (updated_soc - self.current_soc) * self.capacity * 60 / 15 # if charge, positive, if discharge, negative
